...
nfreq=40; % number of frequencies in the spectrum decide the maximum simulation time

freq_cutoff=3;rand_seed=2; 

theta_waves=beta_waves(ii); % incoming wave direction angle in degrees with respect to XZ plane [degrees]

% Getting wave surface elevation parameters from spectrum 

[zeta_A,omega,phase,wavenum,t_sim_max]=create_waves(spectrum_type,hs,T0,depth,nfreq,freq_cutoff,rand_seed);
...
theta_wind=90; % Angle of approach of the wind [degrees] with respect to the XZ plane
...
[A_hub,U_wind,t_wind] = kaimal_sim(I_ref,L_u,U_mean,fs,7*nfreq,rand_seed,N_TWR,L_TWR_total,N_TP_AW,L_TP_AW); % generating wind velocity time series at nodes subjected to wind loading
...
% J_hub=2607890; % Rotary inertia of the rotor nacelle [kg*m^2], from NREL 5
...
[M_PILE_EMBED_xz,K_PILE_EMBED_xz,M_PILE_EMBED_yz,K_PILE_EMBED_yz,M_PILE_EMBED_tor,K_PILE_EMBED_tor]=M_K_section(N_pile_embed,L_pile_embed,p,...
...
[M_system_xz,K_system_xz,N_xz,Nk_xz,N_t,N_tp_aw,N_tp_uw,N_pile_uw,N_wind]=Assemble_M_K(N_TWR,N_TP_AW,N_TP_UW,N_pile_UNembed,N_pile_embed,...
...
% segregating the nodes where hydro dynamic forces will act in yz plane

N_yz=N_xz+size(M_system_xz,1);

% segregating the nodes where spring forces will act in yz plane

Nk_yz=Nk_xz+size(M_system_xz,1);
...
M_system_xz(2,2)=M_system_xz(2,2)+J_y;
...
k_soil=10e6; % Pa/m, taken from the DNV curves assuming angle of friction for sand is 30 degrees

% k_soil=0;

c1=2;c2=2.7;c3=29; % parameters taken from DNV tables

gamma=10000; % specific weight of the soil [N/m^3]

y=0:0.0001:2; % deflections [m]

K_non_lin = non_lin_spring(N_pile_embed,L_pile_embed,k_soil,D_out_pile,c1,c2,c3,gamma,y); % spring tables

% plot(1000*y,K_non_lin(2,:),'LineWidth',2);grid on;ylabel('Soil spring stiffness [N/m]');xlabel('Deflection [mm]');title('p-y curves for sand at a depth of 2.4 [m]')
...
PRM.vel=vel; % wave velocities 

PRM.F_mac=Force_MacCamy; % Maccamy force

PRM.theta_waves=theta_waves; % angle of approach of waves

PRM.N=size(M_system_1,1); % total number of degrees of freedoms in the whole system i.e. 2 plane system after removing the last row and column
...
PRM.F=zeros(PRM.N,1); % creating a null force vector

PRM.t=t1; % time for simulating
...
PRM.Nk_xz=Nk_xz; % index number of the embedded portions in global matrix for xz plane

PRM.Nk_yz=Nk_yz; % index number of the embedded portions in global matrix for yz plane

PRM.Springs=K_non_lin; % non linear spring tables

PRM.y=y; % spring deflections 
...
PRM.N_t=N_t; % Positions in the main vector where wind loads on the tower will act in XZ plane

PRM.N_tp_aw=N_tp_aw; % Positions in the main vector where wind loads on the above water transition piece will act in XZ plane
...
PRM.N_wind=N_wind; % start and end positions of nodes in air in XZ plane
...
tspan=t1; % simuation period

h = waitbar(0,'Please wait...work in progress');

[t_out,q_out] = ode45(@(t,q)ode_func_nonlin(t,q,PRM),tspan,IC);

Q=(q_out(:,1:3*n_modes))';

X_nl_out=v*Q;

X_nl_out_dot=v*(q_out(:,3*n_modes+1:end))';

save(sprintf('x_nl_out_%02d',ii),'X_nl_out');

save(sprintf('x_nl_out_dot_%02d',ii),'X_nl_out_dot');

save(sprintf('t_%02d',ii),'t_out');

figure

plot(t_out,X_nl_out(1,:));
...
plot(t_out,X_nl_out(size(M_system_xz,1)+1,:));
...
% t1=t_out;
% 
% H = anim(L_TWR_total,L_pile_total,L_TP_total,M_system_xz,rpm,L_blade,t1,X_nl_out);
% 
% movie(H)
%  
% movie2avi(H,'mymovie.avi','compression','none');
 % close(h);
